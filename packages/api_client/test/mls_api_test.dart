...
void main() async {
  ...
  group('api client protocol and serde tests', () {
    test('getLessonPlan', () async {
      ...
      await createLessonUnit(apiHost, planId, [
        Frame(
          entities: [
            Entity.measureChart(
      ...
      final lessonPlan = await getLessonPlan(
        apiHost,
        planId,
        httpClient: httpClient,
      );
    ...
    test('getLessonUnit', () async {
      final planId = await createLessonPlan(apiHost, httpClient: httpClient);
      final unitId = await createLessonUnit(apiHost, planId, [
        Frame(
          entities: [
            Entity.measureChart(
              instrument: Instrument.banjo,
      ...
      final (lessonPlan, lessonUnit) = await getLessonUnit(
        apiHost,
        planId,
        unitId,
        httpClient: httpClient,
      ...
      expect(
        lessonUnit.frames[0].entities[0].type,
        equals(EntityType.measureChart),
      );
      expect(
        lessonUnit.frames[0].entities[0].offset,
    ...
    test('updateLessonUnitFrames', () async {
      ...
      final (_, unit) = await getLessonUnit(
        apiHost,
        planId,
        unitId,
        httpClient: httpClient,
