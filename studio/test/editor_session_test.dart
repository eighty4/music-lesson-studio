...
void main() async {
  ...
  final httpClient =
      MlsTokenHttpClient(await createAuthToken(jsDir: '../packages'));
  group('refreshLessonData', () {
    ...
    test('retrieves lesson unit without frames', () async {
      ...
      final unitId = await api.createLessonUnit(apiHost, planId, [],
    ...
    test('retrieves lesson unit with frames', () async {
      ...
      expect(result.unit?.frames[0].entities[0].type,
          equals(EntityType.chordChart));
      expect(result.unit?.frames[0].entities[0].offset,
          equals(frames[0].entities[0].offset));
      expect(result.unit?.frames[0].entities[0].size,
          equals(frames[0].entities[0].size));
      expect(result.unit?.frames[1].entities[0].type,
          equals(EntityType.measureChart));
      expect(result.unit?.frames[1].entities[0].offset,
          equals(frames[1].entities[0].offset));
      expect(result.unit?.frames[1].entities[0].size,
  ...
  group('saveLessonUnitFrames', () {
    ...
    test('creates unit for new lesson of existing plan', () async {
      ...
      final fetched = await api.getLessonUnit(apiHost, planId, result.unit!.id!,
          httpClient: httpClient);
    ...
    test('updates frame data of new lesson', () async {
      ...
      final fetched = await api.getLessonUnit(
          apiHost, planId, result2.unit!.id!,
          httpClient: httpClient);
      expect(fetched.$2.frames.length, equals(2));
    });
  });
